...
local Melee_and_RemoteBuy = {
	...
	["Dragon Claw"] = function()
		local OwnDragonClaw = CommF_:InvokeServer("BlackbeardReward", "DragonClaw", "1") == 1
...
spawn(function()
	...
	for i, v in CommF_:InvokeServer("GetFruits", LocalPlayer.PlayerGui.Main.FruitShop:GetAttribute("Shop2")) do
		if v.OnSale then
	...
	if not fr or fr ~= "Temple Of Time" and fr ~= "Dismension" then
		...
		pcall(
			...
			LocalPlayer.Character:MoveTo(
                    CFrame.new(28390.7812, 14895.8574, 106.534714, 0.0683786646, 1.44424162e-08, - 0.997659445, 7.52342522e-10, 1, 1.45278642e-08, 0.997659445, - 1.74397752e-09, 0.0683786646).Position)
...
function CheckPlayerAlive()
	local a2, b2 = pcall(function()
		return LocalPlayer.Character.Humanoid.Health > 0
	end)
	wait()
	if a2 then
		return b2
	...
	if _G.ServerData['PlayerData'].DevilFruit == '' then
		for i = # fruitsSnipes, 1, 1 do
			...
			if FruitStocks[f] then
				CommF_:InvokeServer("PurchaseRawFruit", f, LocalPlayer.PlayerGui.Main.FruitShop:GetAttribute("Shop2"))
				return
			end
		end
	end
...
function getNextSwordToFarm()
	...
	Swords = {}
	for _, itemData in CommF_:InvokeServer("getInventory") do
...
function checkFruit1M(in5)
	...
	if in5 then
		...
		if FOUNDDF then
			CommF_:InvokeServer("LoadFruit", FOUNDDF)
			wait(.5)
			if fruitsea3bp() then
				return fruitsea3bp()
			end
...
function eatFruit(fruitsSnipes, includedInventory)
	...
	if includedInventory then
		for i, v in fruitsSnipes do
			if _G.ServerData["Inventory Items"][v] then
				CommF_:InvokeServer("LoadFruit", v)
				l4432()
			end
		end
	end
...
function Storef(v)
	if _G.CurrentTask ~= 'Eat Fruit' and _G.CurrentTask ~= 'Auto Sea 3' then
		return CommF_:InvokeServer("StoreFruit", tostring(v:GetAttribute("OriginalName")), v)
	end
end 
...
function CheckMessage(v1)
	...
	elseif v1:find('rare item') then
		_G.Config.FireEssencePassed = typeof(CommF_:InvokeServer("BuyDragonTalon", true)) ~= 'string'
			...
			if not _G.ServerData["PlayerBackpack"][v.Name] then
						...
						if not Storef(v) then
							...
							v:GetPropertyChangedSignal('Parent'):Connect(function()
								if not v.Parent then
									_G.ServerData["PlayerBackpackFruits"][nextid] = nil
								end
							end)
						end
					end
				...
				spawn(function()
					if v:IsA('Tool') and v.ToolTip == 'Melee' then
						repeat
							wait()
						until _G.Config and _G.Config['Melee Level Values']
						_G.Config["Melee Level Values"][v.Name] = v:WaitForChild('Level').Value
					end
				end)
			end
		end
				...
				LocalPlayer.Backpack.ChildAdded:Connect(function(v)
					...
					if v.Name == 'Holy Torch' then
						for i, v in CommF_:InvokeServer("TushitaProgress").Torches do
							if not v then
								spawn(function()
									CommF_:InvokeServer("TushitaProgress", "Torch", i)
								end)
							end
						end
					end
					delay(3, function()
						if v.Name == 'Red Key' then
							print('DOugh chip unlocked: ', CommF_:InvokeServer("CakeScientist", "Check"))
						end
						if v.Name:find('Fruit') then
							if not Storef(v) then
								local nextid = # _G.ServerData["PlayerBackpackFruits"]
					...
					spawn(function()
						if v:IsA('Tool') and v.ToolTip == 'Melee' then
							repeat wait() until _G.Config and _G.Config['Melee Level Values']
							_G.Config["Melee Level Values"][v.Name] = v:WaitForChild('Level').Value
						end
					end)
					for i, v in _G.ServerData["PlayerBackpack"] do
						if not LocalPlayer.Character:FindFirstChild(i) and not LocalPlayer.Backpack:FindFirstChild(i) then
							_G.ServerData["PlayerBackpack"][i] = nil
						end
					end
				end)
...
spawn(function()
	_G.Ticktp = tick()
	getgenv().TushitaQuest = CommF_:InvokeServer("TushitaProgress")
	if CheckEnabling('Cursed Dual Katana') and TushitaQuest and not TushitaQuest.OpenedDoor and _G.ServerData['PlayerData'].Level >= 2000 then
		...
		spawn(function()
			...
			if AutoRipIndraHop then
				for i = 1, 120 do
					if CommF_:InvokeServer("TushitaProgress") and CommF_:InvokeServer("TushitaProgress").OpenedDoor then
						break;
					end
...
function LoadItem(d)
	...
	print('Loaditem', d)
	CommF_:InvokeServer("LoadItem", d)
end 
function EquipWeapon(ToolSe)
	...
	if not ToolSe then
		...
		LocalPlayer.Character.Humanoid:EquipTool(LocalPlayer.Backpack:FindFirstChild(ToolSe))
	end
end 
	...
	local LS, LS2 = pcall(function()
		if IsPlayerAlive() and MobInstance and MobInstance:FindFirstChild("Humanoid") and MobInstance.Humanoid.Health > 0 then
			...
			delay(.01, function()
				repeat
					wait()
				until MobInstance.PrimaryPart and GetDistance(MobInstance.PrimaryPart) < 150
				addCheckSkill(MobInstance)
				wait()
				sethiddenproperty(LocalPlayer, "SimulationRadius", 3300)
				...
				if CommF_:InvokeServer("KenTalk", "Start") == 0 then
			...
			repeat
				...
				wait()
			until not MobInstance or not MobInstance:FindFirstChildOfClass("Humanoid") or not MobInstance:FindFirstChild("HumanoidRootPart") or MobInstance.Humanoid.Health <= 0
...
function CheckSafeZone(p)
	for i, v in workspace["_WorldOrigin"].SafeZones:GetChildren() do
		if v:IsA("Part") then
			if GetDistance(v, p.PrimaryPart) <= 200 and p.Humanoid.Health / p.Humanoid.MaxHealth >= 0.9 then
				return true
			end
		end
	end
...
function CheckQuestByLevel(cq)
	...
	for i, v in Quest do
		for i1, v1 in v do
			...
			for i2, v2 in v1.Task do
				if lvlPl >= lvlreq and lvlreq >= Returner["LevelReq"] and lvlreq <= LevelMaxReq and v1.Task[i2] > 1 and not table.find(UselessQuest, tostring(i)) then
	...
	if DoubleQuest and IsHavingQuest() then
		...
		if lvlPl >= 10 and IsHavingQuest() and CheckCurrentQuestMob() == Returner["Mob"] then
			for i, v in Quest do
				for i1, v1 in v do
					for i2, v2 in v1.Task do
...
function GetQuest(QuestTables)
	...
	if QuestTables.QuestCFrame and GetDistance(QuestTables.QuestCFrame) <= 8 then
		CommF_:InvokeServer("StartQuest", tostring(QuestTables["QuestName"]), QuestTables["QuestId"])
		wait(.75)
	else
...
local function FireAddPoint(PointName, num)
	CommF_:InvokeServer("AddPoint", PointName, num)
end
function autoStats()
...
function FarmMobByLevel(level)
	...
	elseif Sea2 and level >= 1500 then
		level = 1450
		if CommF_:InvokeServer("ZQuestProgress", "Zou") == 0 then
			TeleportWorld(3)
		end
	end
	if _G.KillAuraConnection then
...
function CheckRaceVer()
	local v113 = CommF_:InvokeServer("Wenlocktoad", "1")
	local v111 = CommF_:InvokeServer("Alchemist", "1")
...
function checkExploiting(playerInstance)
	if not playerInstance or playerInstance.Name == LocalPlayer.Character.Name then
		return
	end
			...
			v:GetPropertyChangedSignal('Parent'):Connect(function()
				if not v.Parent or v.Parent ~= workspace["_WorldOrigin"].Locations then
					if _G.CurrentTask == 'Auto Raid' and _G.ServerData['Nearest Raid Island'] then
						repeat
							wait(1)
						until not LocalPlayer.PlayerGui.Main.Timer.Visible
						_G.CurrentTask = ''
						...
						local v302 = CommF_:InvokeServer("Awakener", "Check")
...
function buyRaidingChip()
	if _G.EnLoaded and (Sea2 or Sea3) and tick() - JoinedGame > 60 and tick() - _G.SuccessBoughtTick > 60 and _G.ServerData['PlayerData'].Level >= 1100 and not _G.ServerData["PlayerBackpack"]['Special Microchip'] and not CheckIsRaiding() then
		if (((_G.CurrentTask == '' or _G.MeleeTask == 'None') and _G.CurrentTask ~= 'Auto Sea 3') or _G.FragmentNeeded) and not checkFruit1M() and (_G.FragmentNeeded or (not CheckX2Exp() and ((_G.ServerData['PlayerData'].Fragments < 7500 or (_G.ServerData['PlayerData'].Level >= 2550 and _G.ServerData['PlayerData'].Fragments < 25000)) or # _G.ServerData["PlayerBackpackFruits"] > 0))) then
			...
			local bought = CommF_:InvokeServer("RaidsNpc", "Select", SelRaid) == 1
			if bought then
				...
				if below1MFruit then
					...
					if CommF_:InvokeServer("LoadFruit", below1MFruit) then
						buyRaidingChip()
...
function AddChest(chest)
	...
	if not string.find(chest.Name, 'Chest') and not (chest:IsA('Part') or chest:IsA('BasePart')) then
		return
	end
	local CallSuccess, Returned = pcall(function()
		return GetDistance(chest)
	end)
	if not CallSuccess or not Returned then
		return
	end
...
function HasColor(BrickColorName)
	...
	if not _G.ServerData['PlayerData']['Colors'][RealColorName] then
		...
		CommF_:InvokeServer("activateColor", RealColorName)
		return true
	end
end     
local Item_Bosses = {
...
ThisiSW = RunService.Heartbeat:Connect(function()
	...
	if IsPlayerAlive() then
		...
		if KillingMob or tick() - TOIKHONGBIET < 3 then
			return
		end
		TOIKHONGBIET = tick()
		for i, v in CommF_:InvokeServer("getInventory") do
		...
		if _G.FruitSniping and _G.SnipeFruit and _G.ServerData['PlayerData'].DevilFruit == '' and tick() - LastCheckTickFruit >= 3 then
			...
			if tick() - CheckFruitStockTick >= 900 then
				CheckFruitStockTick = tick()
				_G.ServerData["Fruits Stock"][1] = (function()
					local returnTable = {}
					for i, v in CommF_:InvokeServer("GetFruits", false) do
						if v.OnSale then
				...
				_G.ServerData["Fruits Stock"][2] = (function()
					local returnTable = {}
					for i, v in CommF_:InvokeServer("GetFruits", true) do
						if v.OnSale then
							table.insert(returnTable, v.Name)
						end
					end
			...
			for i, v in _G.FruitSniping do
				if table.find(_G.ServerData["Fruits Stock"][1], v) then
					...
					CommF_:InvokeServer("PurchaseRawFruit", v, false)
					break;
				elseif table.find(_G.ServerData["Fruits Stock"][2], v) then
					print("Buying: " .. tostring(v))
					CommF_:InvokeServer("PurchaseRawFruit", v, true)
					break;
				end
			end
		...
		if not _G.RaceV4Progress or (_G.RaceV4Progress ~= 4 and _G.RaceV4Progress ~= 5) then
			_G.RaceV4Progress = CommF_:InvokeServer("RaceV4Progress", "Check")
		end
		...
		if not _G.ServerData['PlayerData']["Elite Hunted"] or _G.ServerData['PlayerData']["Elite Hunted"] < 30 then
			_G.ServerData['PlayerData']["Elite Hunted"] = tonumber(CommF_:InvokeServer("EliteHunter", "Progress")) or 0
		end
		_G.ServerData['PlayerData']["RaceVer"] = CheckRaceVer()
		for i, v in CommF_:InvokeServer("getColors") do
			if v["Unlocked"] then
				_G.ServerData['PlayerData']['Colors'][v.HiddenName] = v
			end
		end
		CommF_:InvokeServer("Cousin", "Buy")
		local a, b = CommF_:InvokeServer("ColorsDealer", "1")
		if a and b and b > 5000 then
