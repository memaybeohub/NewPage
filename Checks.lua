...
getgenv().refreshTask = function() 
    if _G.TOIKHONGBINGU then 
        ...
        if _G.CurrentTask == '' then  
            ...
            elseif _G.ServerData['PlayerData'].Level > 200 and CheckEnabling('Saber') and not _G.Config.OwnedItems["Saber"] then 
...
AutoSoulGuitar = function() 
    ...
    if not _G.SoulGuitarPuzzlePassed then 
        if not CurrnetPuzzle then  
            ...
            if not Sea3 then 
                TeleportWorld(3)
            elseif (moonPhase == 5 or moonPhase == 4) and (moonPhase == 4 or (moonPhase == 5 and (game.Lighting.ClockTime > 15 or game.Lighting.ClockTime < 5))) then   
